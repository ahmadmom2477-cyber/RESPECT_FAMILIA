...
- Live status fetched from Kick.com via Apify, cached 5 minutes per streamer

## Stack
- Frontend: React + Vite (`artifacts/respect-family`)
- Backend: Express (`artifacts/api-server`)
- Database: **MongoDB Atlas** (Mongoose ODM, `lib/db`)
...
- Email allowlist managed in MongoDB collection `admin_emails` via Admin Dashboard → "Admins" tab.
...
- `favorites` — (userId, username) pairs, unique compound index
- `admin_emails` — admin allowlist seeded with `3x51hb@gmail.com`
- `counters` — auto-increment numeric IDs (preserves the integer-id API contract)

Schemas live in `lib/db/src/models/*.ts`. The API server calls `connectMongo()` on boot (`artifacts/api-server/src/index.ts`).
...
- Cache TTL: 5 minutes per streamer in MongoDB.
...
**Build command:**
```
corepack enable && pnpm install --frozen-lockfile && pnpm --filter @workspace/api-server run build && pnpm --filter @workspace/respect-family run build
```
**Start command:**
```
...
- `MONGODB_URI` — MongoDB Atlas connection string (e.g. `mongodb+srv://user:pass@cluster.mongodb.net/respect_family?retryWrites=true&w=majority`)
...
- `BASE_PATH=/` — set to `/` when deploying at the root domain (required at build time)
- `ADMIN_EMAILS` — (optional) bootstrap admin emails, comma-separated
- `NODE_ENV=production`

**Notes:**
- `PORT` is no longer required at build time (defaults to 5000 for the dev server). Render sets `PORT` automatically at runtime.
- The Express server serves the built Vite frontend (`artifacts/respect-family/dist/public`) automatically in production.
